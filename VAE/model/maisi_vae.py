...
class VAE(nn.Module):
    ...
    def __init__(
        ...
        if any((out_channel % norm_num_groups) != 0 for out_channel in channels):
            raise ValueError("VAE expects all num_channels being multiple of norm_num_groups")

        if len(channels) != len(attention_levels):
            raise ValueError("VAE expects num_channels being same size of attention_levels")
    ...
    def load_old_state_dict(self, old_state_dict: dict, verbose=False) -> None:
        """
        Load a state dict from an VAE trained with [MONAI Generative](https://github.com/Project-MONAI/GenerativeModels).

        Args:
            old_state_dict: state dict from the old VAE model.
        """
...
# -------------------------------------------------------------------------
#      If you haven't defined these Maisi* classes (MaisiEncoder, etc.)
#      in your code yet, be sure to include them before this snippet.
#      (Refer to your original code for the Maisi* definitions.)
# -------------------------------------------------------------------------
...
class VAE_Lite(nn.Module):
    ...
    def __init__(
        ...
        in_channels: int = 1,
        out_channels: int = 1,
        # Fewer channels and fewer levels for memory savings
        channels: Sequence[int] = (64, 128),
        # Just one residual block per level here
        num_res_blocks: Sequence[int] = (1, 1),
        # No self-attention blocks in the main levels
        ...
        norm_eps: float = 1e-5,
        # Turn off nonlocal attention blocks to save memory
        with_encoder_nonlocal_attn: bool = False,
        with_decoder_nonlocal_attn: bool = False,
        # For training speed/memory usage, set this to True only if needed
        use_flash_attention: bool = False,
        # No fancy linear layers for QKV
        include_fc: bool = False,
        use_combined_linear: bool = False,
        # For the decoder, can be True or False depending on your preference
        use_convtranspose: bool = False,
 ...
 # Instead of: num_channels=channels
# Use:        channels=channels
        self.encoder = Encoder(
            spatial_dims=spatial_dims,
            in_channels=in_channels,
            channels=channels,  # <-- rename here
            out_channels=latent_channels,
            num_res_blocks=num_res_blocks,
            norm_num_groups=norm_num_groups,
            norm_eps=norm_eps,
            ...
            use_combined_linear=use_combined_linear,
            use_flash_attention=use_flash_attention,
            # remove or rename any extras you had for 'num_splits' or 'dim_split'
            # if your Encoder definition doesn't have them
        )

# Instead of: num_channels=num_channels
# Use:        channels=channels
        self.decoder = Decoder(
            ...
            use_flash_attention=use_flash_attention,
            # remove or rename any extras you had for 'num_splits' or 'dim_split'
            # if your Decoder definition doesn't have them
        )


        # 2) Middle block: typically K/V/μ/logσ computations for KL
        #    Here we replicate a smaller version of the standard AutoencoderKL middle part.
        #    Instead of a big FC, we typically do a simple 1x1 conv for mean/logvar.
        self.conv_mean = nn.Conv3d(latent_channels, latent_channels, 1)
        self.conv_var = nn.Conv3d(latent_channels, latent_channels, 1)
    ...
    def encode(self, x: torch.Tensor):
        """
        Encode step: pass data through the encoder to produce mean and logvar.
        """
        h = self.encoder(x)  # shape: [B, latent_channels, D', H', W']
        mean = self.conv_mean(h)
        logvar = self.conv_var(h)
        return mean, logvar

    def reparameterize(self, mean: torch.Tensor, logvar: torch.Tensor):
        """
        Reparameterization trick: sample z from N(mean, std^2).
        """
    ...
    def decode(self, z: torch.Tensor):
        """
        Decode step: pass sampled latents through the decoder.
        """
